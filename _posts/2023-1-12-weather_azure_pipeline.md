...
A lot of this was inspired and was made possible by following a tutorial by Nick
https://www.youtube.com/watch?v=yu013hpJvm8&list=PLX_uXCDfdJzhzrY2OvyLkmeyDAF2LnX0R&index=2
and others who have made things easy

## Introduction

I recently tackled a project where I set up a data processing workflow on Azure. Below are the details.

## Cloud Setup

### 1. Linux VM Setup

First things first, I set up a Linux Virtual Machine. I made sure you can easily connect to it using Remote Desktop Protocol (RDP). This VM will serve as the round-the-clock retriever of data (more on this later).

### 2. Datalake

It's like a storage space where we can neatly organize and manage all the files that come in.

### 3. Azure SQL Server

After the data is cleaned, it can be stored on an Azure SQL Server. It can now we used for analytics, ML, or reporting purposes.
<br><br>
...
Using the , I pulled in weather data right from my computer. Check out the code snippet below for how I did it:
...
### 1. Processing the Data in Azure Data Factory

I put together the following pipeline in Azure Data Factory to first grab new files, process the data in them, put the processed data back into a different folder in the datalake, and finally grab the processed data from there and put it into the Azure SQL database. 

![Data Processing Pipeline](link_to_image)

#### 1.a Cleaning the data in a Databricks Notebook
The first step in the pipeline is to send the data to a Databricks notebook, where it will be cleaned, and pushed back in a different folder in the datalake - the "cleaned" folder.
...
Looking ahead, our focus is on creating a simple Flask dashboard. This dashboard will give us a quick look at what's happening with the data trends in Berlin, updating in real-time.
